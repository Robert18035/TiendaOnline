...
class ChamarrasActivity : AppCompatActivity() {

    private var adapter: FirebaseListAdapter<Producto>? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_chamarras)
        mostrarChamarras()
    }

    private fun mostrarChamarras  () {
        val listaArticulos: ListView =
            findViewById(R.id.lista_chamarras) as ListView
        val query: Query =
            FirebaseDatabase.getInstance()
                .reference.child("Productos").orderByChild("Categoria")  .equalTo("Chamarra")     //Accesa a la base de datos
